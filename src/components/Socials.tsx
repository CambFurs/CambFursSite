...
const Socials = () =>  (
  <section id="socials" className={`${styles.flexCenter} flex-row flex-wrap sm:mb-20 mg-6`}>
    {socialmedia.map((social) => (
      <div key={social.id} className={`flex-1 justify-start items-center flex-row m-3`}>
        <a href={social.link} target='_blank' rel='noreferrer' className='container justify-center'>
          ...
          <p className='font-poppins font-semibold xs:text-[40px] text-[30px] xs:leading-[53px] leading-[43px] dark:text-white text-black'>{social.name}</p>
        </a>
      </div>
    ))}
  </section>
